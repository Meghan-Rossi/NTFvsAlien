...
/datum/job/som/squad/radio_help_message(mob/M)
	...
	if(istype(SSticker.mode, /datum/game_mode/hvh/combat_patrol))
		if(issensorcapturegamemode(SSticker.mode))
			...
			to_chat(M, span_userdanger("Your platoon has orders to patrol a remote territory illegally claimed by corpos. Intel suggests NTF units are similarly trying to press their claims by force. Work with your team and eliminate all NTF you encounter while preserving your own strength! High Command considers wiping out all enemies a major victory, or inflicting more casualties a minor victory."))
		return
